...
def data_generation_dense_mixed_endo(num_samples, num_cont_imp, num_disc_imp, num_cont_unimp, num_disc_unimp, std=1.5,
    ...
    xc = np.random.normal(1, std, size=(num_samples, num_cont_imp)) #+ np.random.lognormal(1, 0.25, size=(num_control, num_cov_dense))   # data for conum_treatedrol group
...
def dgp_poly_no_interaction(n_samples, n_imp, n_unimp):
    x_imp = np.random.normal(1, 1.5, size=(n_samples, n_imp))
    t_imp = np.random.binomial(1, 0.5, size=(n_imp,))
    t = np.random.binomial(1, 0.5, size=(n_samples,))
    t_eff_powers = np.random.randint(1, 4, size=(n_imp,))
    eff_sign = np.random.choice([-1, 1], n_imp)
    y0 = np.sum(np.cos(x_imp), axis=1) + np.sum((1 / (1 + np.exp(-x_imp))), axis=1) - np.sum((x_imp**3), axis=1) - 2
    y1 = np.sum(np.cos(x_imp), axis=1) + np.sum((1 / (1 + np.exp(-x_imp))), axis=1) - np.sum((x_imp**3), axis=1) - 2 + np.sum(np.abs(((x_imp * t_imp) ** t_eff_powers)), axis=1)
...
# def dgp_poly_no_interaction(n_samples, n_imp, n_unimp):
#     x_imp = np.linspace(-1, 2, n_samples).reshape(-1, 1)
#     # x_imp = np.random.uniform(-3, 5, size=(n_samples, n_imp))
#     # x_imp = np.random.uniform(0, 1, size=(n_samples, n_imp)) * [1,2,3,1,2,3,1,2,3,1,2,3,1,2,3,1]
#     t_imp = np.random.binomial(1, 0.5, size=(n_imp,))
#     # t_imp = [1] * n_imp
#     t = np.random.binomial(1, 0.5, size=(n_samples,))
#
#     eff_sign = np.random.choice([-1, 1], n_imp)
#     # eff_sign = [1, 1, 1, 1, 1, 1, -1, -1]
...
#     # t_eff_sign = [1, 1, 1, 1, 1, 1, -1, -1]  # keep everything positive for now to limit issues with small TE values
