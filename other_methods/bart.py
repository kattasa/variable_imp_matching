...
def bart(outcome, treatment, data, n_splits=2, result='brief', gen_skf=None, random_state=0):
    ...
    for est_idx, train_idx in gen_skf:
        ...
        bart_res_c = dbarts.bart(Xc, Yc, Xtest, keeptrees=True, verbose=False, ntree=40, ndpost=200,
                                 seed=random_state)
        if discrete_outcome:
            y_c_hat_bart = norm.cdf(bart_res_c[2]).mean(axis=0)
        else:
            y_c_hat_bart = np.array(bart_res_c[7])
        bart_res_t = dbarts.bart(Xt, Yt, Xtest, keeptrees=True, verbose=False, ntree=40, ndpost=200,
                                 seed=random_state)
        if discrete_outcome:
